...
void DAC_Init(void){ volatile unsigned long delay;
  ...
  delay = SYSCTL_RCGC2_R;    // allow time to finish PortB clock activating
  GPIO_PORTB_DIR_R |= 0x3F;      // make PB5-0 out
	GPIO_PORTB_DR8R_R |= 0x3F;      // set outputs to handle 8 mA
	GPIO_PORTB_AFSEL_R &= ~0x3F;      // not alternative
  GPIO_PORTB_AMSEL_R &= ~0x3F;      // no analog
  GPIO_PORTB_PCTL_R &= ~0x000FFFFF; // bits for PB5-0
